 ...
 *
 */
public class EECEPlannerAlgorithmExecutorDuplication {

	public static void main(String[] args) throws Exception {

		// First step: Initialize the WorkflowSim package.
		List<String> daxPaths = null;
		String prefixPath = "E:/实验数据/Montage/";
		...
		for (int i = 0; i < 3; i++) {
			...
			for (int k = 0; k < 3; k++) {
				daxPaths.add("E:/src/JavaEE/WorkflowSim4/Montage_25.xml");
			}
...
//			Parameters.getSchedulesList().clear();
//			Parameters.timePoints.clear();
//			Parameters.inputDataLocations.clear();
			Parameters.clear();
				...
				for(int m = 0;m < rankMethods.length; m++){
					for(int n = 0; n < deadlinelevels.length; n++){
						for(int t = 0; t < kmaxs.length; t++){
							...
							//调用仿真函数
							doSimulations(daxPaths, workflowMethods[l], rankMethods[m], deadlinelevels[n], kmaxs[t]);
							long currentTime = System.currentTimeMillis();
							
							//秒为单位
							long runtime = (currentTime - beginTime) / 1000;
							result.setWorkflowNumber(3);
							result.setTaskNumber(25);
							result.setInstanceNumber(i);
	...
	protected static void doSimulations(List<String> daxPaths, WorkflowSequeningMethod workflowMethod,
		...
		Map<Integer, List<CondorVM>> dcToVMs = VMAllocationPolicyImpl.bindVMToDatacenter(dcList, vmList);
