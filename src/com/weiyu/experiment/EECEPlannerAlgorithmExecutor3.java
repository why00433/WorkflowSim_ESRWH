 ...
 *
 */
public class EECEPlannerAlgorithmExecutor3 {
			...
			for (int j = 0; j < taskNumbers.length; j++) {
				for (int k = 0; k < 10; k++) {
					...
					for (int o = 0; o < 5; o++) {
						...
						// for(int n = 0; n < deadlinelevels.length;
						// n++){
						for (int t = 0; t < kmaxs.length; t++) {
							SimulationResult result = new SimulationResult();
							long beginTime = System.currentTimeMillis();
							// 调用仿真函数
							doSimulations(daxPaths, workflowMethods[1], rankMethods[2], deadlinelevels[0], kmaxs[t]);
	...
	private static void exportToTxt(List<SimulationResult> results) throws IOException {
		String filePath = "E:/实验数据/Montage_results_VND_20180603.txt";
