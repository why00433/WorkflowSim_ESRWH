...
/**
 * 多数据中心场景下多工作流实例 进行参数校正 1）用VND的算法和不用VND算法之间的进行对比。 2）在不同的截止期情况下进行比较
 * 3）在不同的工作流实例规模的情况下进行比较 4）在不同的任务数量规模性进行比较
 * 
 * @author Wei Yu
 *
 */
public class EECEPlannerAlgorithmExecutor4 {

	public static void main(String[] args) throws Exception {

		// First step: Initialize the WorkflowSim package.
		List<String> daxPaths = null;
		String prefixPath = "E:/实验数据/CyberShake/";
		...
		Parameters.DeadlineLevel[] deadlinelevels = { Parameters.DeadlineLevel.D1, Parameters.DeadlineLevel.D2,
				...
				Parameters.DeadlineLevel.D6, Parameters.DeadlineLevel.D7, Parameters.DeadlineLevel.D8,
				Parameters.DeadlineLevel.D9, Parameters.DeadlineLevel.D10 };

		Parameters.VNDMethod[] vndMethods = { Parameters.VNDMethod.VND, Parameters.VNDMethod.NOVND };
...
//					for (int o = 0; o < 5; o++) {
						...
						for (int n = 0; n < deadlinelevels.length; n++) {
							...
							for (int w = 0; w < vndMethods.length; w++) {
								...
								// 调用仿真函数
								doSimulations(daxPaths, workflowMethods[1], rankMethods[2], deadlinelevels[n],
...
//		}
	...
	private static void exportToTxt(List<SimulationResult> results) throws IOException {
		String filePath = "E:/实验数据/comparision_result_20180615.txt";
...
//					(elecCostForAllWorkflowsBeforeVND - minimalElecCostBeforeVND) / minimalElecCostBeforeVND * 100);
	...
	protected static void doSimulations(List<String> daxPaths, WorkflowSequeningMethod workflowMethod,
		...
		if (null == vmList || vmList.size() == 0) {
			String vmListPath = "E://实验数据/VM配置_20180601.xls";
