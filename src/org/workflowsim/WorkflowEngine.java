...
public final class WorkflowEngine extends SimEntity {
    ...
    public List<Datacenter> getAllDatacenterList(){
        if(this.datacenterList != null && !this.datacenterList.isEmpty()){
            return this.datacenterList;
        }
        else{
            List<Datacenter> list = new ArrayList<>();
            for(int i = 0;i < getSchedulers().size();i ++){
                list.addAll(getScheduler(i).getDatacenterList());
            }
            return list;
        }
    }
    
    public List<Integer> getAllDatacenterIdsList(){
        if(this.datacenterIdsList != null && !this.datacenterIdsList.isEmpty()){
            return this.datacenterIdsList;
        }
        else{
            List<Integer> list = new ArrayList<Integer>();
            for(int i = 0;i < getSchedulers().size();i ++){
                list.addAll(getScheduler(i).getDatacenterIdsList());
            }
            return list;
        }
    }
     ...
     * @param schedulerId
     */
    public void submitDatacenters(Datacenter datacenter, int schedulerId) {
        getScheduler(schedulerId).submitDatacenters(datacenter);
    }
