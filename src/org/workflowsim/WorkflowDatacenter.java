...
//                            	long bw2 = getVmAllocationPolicy().getHost(Integer.parseInt(site), userId).getVm(Integer.parseInt(site), userId).getBw();
//                                bwth = Math.min(bw, bw2);
//                                //bwth = dcStorage.getBandwidth(Integer.parseInt(site), vmId);
                                
                                //本实验不需要考虑调度阶段的，因此不用考虑这些
                                bwth = vm.getBw();
                            }
                            if (bwth > maxBwth) {
                                maxBwth = bwth;
